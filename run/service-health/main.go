...
func refreshReadinessConfig() error {
	...
	isHealthy, _ = readHealth(instanceId)
	writeHeartbeat()
	return nil
}
...
func listInstances() ([]string, error) {
	...
	for {
		...
		id := strings.Split(attrs.Name, "-")
		if len(id) < 2 {
			continue
		}
		instances[id[1]] = struct{}{}
	}
...
func deleteInstance(instanceId string) {
	...
	objHeartbeat := bucket.Object("beat-" + instanceId)
